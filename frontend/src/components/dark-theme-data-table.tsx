...
export default function DataTableDemo() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/getUserProjects`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 404) {
        ...
        const sortedProjects = result.allUserProjects.sort(
          (
  ...
  const deleteProject = async (projectID: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/deleteProject`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const columns: ColumnDef<Project>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="text-md">{row.getValue("ideaTitle")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-md">{row.getValue("dateCreated")}</div>
      ),
    },
    {
      id: "actions",
        ...
        return (
          <AlertDialog>
            ...
            <AlertDialogContent className="bg-gray-800 text-gray-100">
              ...
              <AlertDialogFooter>
                <AlertDialogCancel className="bg-gray-700 text-gray-100">
                  Cancel
                </AlertDialogCancel>
                <AlertDialogAction
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  if (!isLoaded || !isSignedIn) {
    return (
      <div className="min-h-fit w-full bg-black text-gray-100 p-8 flex justify-center items-center">
        Loading...
      </div>
    );
  }
          ...
          <>
            <div className="mb-4 flex justify-between items-center">
              <Input
                ...
                value={
                  (table.getColumn("ideaTitle")?.getFilterValue() as string) ??
                  ""
                }
                onChange={(event) =>
                  table
                          ...
                          <TableRow key={headerGroup.id}>
                            {headerGroup.headers.map((header) => {
                              return (
                                <TableHead
                                  key={header.id}
                                  className="py-2 text-white text-left"
                                >
                        ...
                        {table.getRowModel().rows?.length ? (
                          table.getRowModel().rows.map((row) => (
                            <TableRow
                              ...
                              onClick={(event) =>
                                handleRowClick(event, row.original.projectID)
                              }
                            ...
                            >
                              {row.getVisibleCells().map((cell) => (
                                <TableCell
                                  key={cell.id}
                                  className="pl-6 text-left font-script text-sm"
                                >
                                  {cell.column.id !== "actions" ? (
                                    <TooltipProvider>
                                      <Tooltip>
                                        {" "}
                                        {/* Added side prop here */}
                                        ...
                                        <TooltipContent className="font-script bg-gray-800 text-gray-100 w-96 max-h-[50%] overflow-y-auto border border-gray-600 rounded-md">
                                          {" "}
                                          {/* Updated TooltipContent here */}
                                          <p className="p-1 text-sm break-words">
                                            {row.original.ideaDescription}
                                          </p>
                                        </TooltipContent>
                                      </Tooltip>
                                    </TooltipProvider>
