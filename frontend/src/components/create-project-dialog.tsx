  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/createProject`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="font-script border border-gray-600 text-sm h-10 w-auto px-6 rounded-xl bg-gray-900 text-white hover:bg-gray-800 font-medium">
          Create Project
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[40%] bg-zinc-900 text-gray-100">
        <DialogHeader>
          <DialogTitle className="text-xl font-script">
            Create Project
          </DialogTitle>
        ...
        <div className="grid gap-4 py-2">
          <Input
            ...
            className="font-script text-base col-span-3 bg-gray-800 text-gray-100 border-gray-600 w-full h-fit rounded-lg px-2"
            maxLength={200}
            required
            placeholder="Enter idea title(upto 200 characters)"
          />
          <Textarea
            id="ideaDescription"
            value={ideaDescription}
            onChange={(e) => setIdeaDescription(e.target.value.slice(0, 2000))}
            className="font-script !text-base col-span-3 bg-gray-800 text-gray-100 border-gray-600 w-full h-[40vh] rounded-lg"
            maxLength={2000}
            required
            placeholder="Enter idea description(upto 2000 characters)"
          />
        </div>
        <DialogFooter>
          <div className="relative group flex w-full justify-center mx-auto">
